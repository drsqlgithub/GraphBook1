...
INSERT INTO IMDB.person WITH (TABLOCKX) (PersonNumber, Primaryname,Birthyear) 
SELECT Name.Nconst, Name.PrimaryName, Name.BirthYear
FROM   IMDBStaging.Staging.Name
--WHERE  Name.Nconst LIKE @peopleMask;

INSERT INTO Imdb.Title WITH (TABLOCKX) (TitleNumber, TitleType, Name, StartYear)
SELECT TConst, TitleType, PrimaryTitle AS Name, TRY_CAST(StartYear AS int)
FROM   IMDBStaging.Staging.BasicDetail

INSERT INTO IMDBInterface.ContributedTo_Person_to_Title WITH (TABLOCKX) (PersonNumber, TitleNumber, ContributionType)
SELECT Tconst AS TitleNumber, CAST(value AS varchar(10)) AS personNumber, 'Director' AS ContributionType
FROM   IMDBStaging.Staging.Crew
		CROSS APPLY STRING_SPLIT(Directors,',')
WHERE value <> '\N'
  AND  EXISTS (SELECT *
			  FROM   Imdb.Title
			  WHERE  Crew.Tconst = Title.TitleNumber)
  --AND Crew.Tconst LIKE @TitleMask
  --AND Value LIKE @peopleMask


INSERT INTO IMDBInterface.ContributedTo_Person_to_Title WITH (TABLOCKX) (PersonNumber, TitleNumber, ContributionType)
SELECT Tconst AS TitleNumber, CAST(value AS varchar(10)) AS personNumber, 'Writer' AS ContributionType
FROM   IMDBStaging.Staging.Crew
...
INSERT INTO IMDBInterface.ContributedTo_Person_to_Title WITH (TABLOCKX) (PersonNumber, TitleNumber, ContributionType)
SELECT DISTINCT Principal.Nconst AS PersonNumber,Principal.Tconst AS TitleNumber, Principal.Category AS ContributionType
FROM   IMDBStaging.Staging.Principal
WHERE  EXISTS (SELECT *
			  FROM   Imdb.Title
			  WHERE  Principal.Tconst = Title.TitleNumber)

...
USE [master]
RESTORE DATABASE [AdventureWorksLT2019] FROM  DISK = N'C:\Users\Public\Downloads\AdventureWorksLT2019.bak' WITH  FILE = 1,  MOVE N'AdventureWorksLT2012_Data' TO N'C:\Program Files\Microsoft SQL Server\MSSQL15.SQLEXPRESS\MSSQL\DATA\AdventureWorksLT2012.mdf',  MOVE N'AdventureWorksLT2012_Log' TO N'C:\Program Files\Microsoft SQL Server\MSSQL15.SQLEXPRESS\MSSQL\DATA\AdventureWorksLT2012_log.ldf',  NOUNLOAD,  STATS = 5
...
INSERT INTO SalesGraph.Customer(CustomerId, Label)
select CustomerId, Concat(FirstName, ' ', LastName,' ',CustomerId) as Label
from   SalesLT.Customer
ORDER BY Label

--Look at a few rows of the output, you see
select top 10 $node_id, CustomerId
...
select NODE_ID_FROM_PARTS(OBJECT_ID('SalesGraph.Customer'), CustomerId),CustomerId, Concat(FirstName, ' ', LastName,' ',CustomerId) as Label
from   SalesLT.Customer;
...
INSERT INTO SalesGraph.Customer($Node_id, CustomerId, Label)
select NODE_ID_FROM_PARTS(OBJECT_ID('SalesGraph.Customer'), CustomerId),CustomerId, Concat(FirstName, ' ', LastName,' ',CustomerId) as Label
from   SalesLT.Customer;

--and the same for product (same duplicate name concern as for customer)

INSERT INTO SalesGraph.Product($Node_id, ProductId, Label)
select NODE_ID_FROM_PARTS(OBJECT_ID('SalesGraph.Product'), ProductId),ProductId, Concat(Name,' ',ProductId) as Label

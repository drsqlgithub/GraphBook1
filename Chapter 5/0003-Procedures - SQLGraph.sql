...
In the next chapter I will use this function on a few different implementations of trees to compare how much time each takes in much larger trees.

The next thing to be a part of the database will be the ability to aggregate over the tree. For example, say we have the following sales figures:
...
We need to be able to say that Tennessee HQ had $3 in sales, Maine 2$ and then the overall organization had $5 in sales. Looking at the hierarchy in the text above, you might thing this is a pretty simple operation. Just fetch the hiearchy's lower level, then get the parents and sum up, then the next parents and sum up. Problem is, hiearchies are rarely going to be so balanced in the real world. The way we process this is actually one where the process is going to take a recursive operation and then flip it into a relational process.

The basis of this operation is an expanded hierarchy. This is basically taking every node that is a parent in the hierarchy and fetching their entire hierarchy. It uses  code we had used before, but instead of filtering to one node, you filter it to one starting node, but every child node will then also include its own hierarchy.
*/
...
(
	...
	from   ExpandedHierarchy
	JOIN [SqlGraph].[Company$ReturnHierarchy](@DisplayFromNodeName) AS CompanyHierarchyDisplay
...
--aggregate each Company for the Company
   ...
   Aggregations
AS (SELECT FilterAndSweeten.ParentCompanyId,
